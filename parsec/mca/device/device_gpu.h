...
struct parsec_gpu_task_s {
    ...
    /* The pending heap and the stream queues both order wrappers directly and
     * therefore cannot follow ec to read parsec_task_t::priority. Inherited
     * from ec when negative on entry into parsec_device_kernel_scheduler(),
     * and refreshed before each stream insertion.
     */
    int32_t                                priority;
